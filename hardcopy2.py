...
import qrcode
import pyzbar.pyzbar as pyzbar # pyzbar is only required for reading / restoring a backup
import PIL
...
def main(argv: list) -> None:
	...
	if (arguments.interactiverestore):
		"""nimport pygame.camera
		import pygame.image

		pygame.camera.init()
		cameras = pygame.camera.list_cameras()

		webcam = pygame.camera.Camera(cameras[0], (1280,720), "RGB")
		webcam.start()
		while (True):
			image = PIL.Image.frombytes('RGB', (1280,720), pygame.image.tostring(webcam.get_image(), 'RGB', False))
		#image.show()
			print(f"{[n.data.decode('ascii') for n in pyzbar.decode(image)]}");"""

		image = PIL.ImageOps.grayscale(PIL.ImageOps.posterize(PIL.ImageOps.autocontrast(PIL.Image.open("20201219_205850.jpg"), cutoff=20), 1)).rotate(270, expand=True)
		print(f"{[n.data.decode('ascii') for n in pyzbar.decode(image)]}");
		image.show()
		return
	...
	document = _printTextToDocument(document, _border + 130, 85, f"Identifier (e.g. filename): { id }\n" \
		...
		f"MD5 hash of restored data:  { hashlib.md5(data).hexdigest() }")
	
	qr = qrcode.QRCode(error_correction=qrcode.ERROR_CORRECT_Q, border=0, box_size=16)
	...
	for i in range(requiredBlocks):
		...
		try: # oh god why
			...
			try: # oh go why²
				qr = qrcode.QRCode(error_correction=qrcode.ERROR_CORRECT_M, border=0, box_size=16)
				qr.add_data(b64EncodedDataBlock)
				qr.make(True);
				print("\t\t... using Error Correction Level M (15% recoverable)")
			except (qrcode.exceptions.DataOverflowError):
				print("\t\t... unable to create QR-Code, would require to low error correction")
				return
		...
		qrAsImage = ImageReader(qr.make_image().get_image())
		document.drawImage(
			qrAsImage, _border + 17.5,
			A4[1] - (42.5 + _fontsize + (A4[0] - (2 * _border + 35))),
			A4[0] - (2 * _border + 35),
			A4[0] - (2 * _border + 35))
		...
		for lineCount in range(requiredLines):
			...
			document.drawString(_border, A4[1] - (505 + ((_fontsize + 2) * lineCount)), f"{(lineCount + 1):02d}")
			...
			for blockCount in range(10):
				...
				line += f"{block} "
			document.drawString(_border + 13.5, A4[1] - (505 + ((_fontsize + 2) * lineCount)), line) # TODO: Replace magic number 396px with dynamic stuff
			...
			document.setFillAlpha(0.4)
			document.drawRightString(A4[0] - _border, A4[1] - (505 + ((_fontsize + 2) * lineCount)), crc32)
			document.setFillAlpha(1)
